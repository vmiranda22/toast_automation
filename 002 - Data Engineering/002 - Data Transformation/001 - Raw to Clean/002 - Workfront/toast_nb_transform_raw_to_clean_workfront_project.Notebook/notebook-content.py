...
# Raw-to-Cleaned Workfront Data Pipeline (Schema Trust-Aware Ingestion)
# 
# This notebook implements a PySpark-based data ingestion pipeline that transforms raw Workfront data 
# into cleaned Delta tables within Microsoft Fabric. It supports schema trust enforcement, type inference, 
# and incremental ingestion based on cursor tracking.
# 
# Key Capabilities:
# - Loads raw 'base' and 'custom' Workfront data from Delta tables.
# - Applies timestamp sanitization and filters records based on an incremental ingestion cursor.
# - Cleans and standardizes data using:
#     - Trust-based type casting from a schema trust table.
#     - Type inference for unverified columns (timestamps, booleans, numerics).
#     - Row hashing for change detection.
# - Aligns cleaned data schema with existing target Delta tables.
# - Attempts safe overwrite only if all type mismatches are pre-approved in the trust table.
# - Falls back to SQL MERGE using row_hash-based updates if overwrite is not safe.
# - Automatically promotes unverified schema entries to verified status when inferred types 
#   consistently match expected types.
# - Updates the ingestion cursor table with the latest processed timestamp.
# 
# Trust Schema Behavior:
# - A trust table tracks (object_type, column_name, data_type, is_verified).
# - If the trust table does not exist, it is initialized empty.
# - Unverified types are inferred and logged, and are now automatically promoted if matching types 
#   are repeatedly observed (simple promotion; a more conservative audit-based strategy may follow).
# 
# Recommended Flow:
# 1. Run the pipeline to log new schema changes.
# 2. Let the pipeline auto-promote trust entries with consistent matches.
# 3. Monitor trust table if additional manual controls are desired.
# 
# This notebook is designed to support robust, safe, and scalable ingestion of Workfront data 
# into analytics-ready tables within Microsoft Fabric.


# MARKDOWN ********************

# Run the code below when the workfront_schema_trust is deleted.

# CELL ********************

# MAGIC %%sql
# MAGIC UPDATE delta.`abfss://b08d383a-b8cc-4b8e-b189-d9d696a01977@onelake.dfs.fabric.microsoft.com/19d0a156-0ba5-48d9-ad24-2d55e56083dc/Tables/dbo/workfront_schema_trust`
# MAGIC SET is_verified = true
# MAGIC WHERE object_type = 'project';
