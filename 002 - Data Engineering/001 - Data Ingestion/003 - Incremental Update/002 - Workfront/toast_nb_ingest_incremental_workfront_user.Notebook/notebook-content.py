...
"""
Workfront User Ingestion Pipeline - Microsoft Fabric (Delta Lake)

Overview:
---------
This PySpark notebook ingests updated `user` records from the Workfront API,
...
object_type = "user"  # The Workfront object being ingested.
...
logger.info(f"Fetching users updated since: {last_update}")
...
if not data:
    ...
    mapping_df = generate_schema_mapping_df(spark, object_type="user", raw_to_sanitized=raw_to_sanitized)
    ...
    log_schema_to_table(spark, object_type="user", raw_to_sanitized=raw_to_sanitized)
