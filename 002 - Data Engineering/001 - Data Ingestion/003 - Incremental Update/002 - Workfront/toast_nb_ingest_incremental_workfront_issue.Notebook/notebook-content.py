...
"""
Workfront Issue Ingestion Pipeline - Microsoft Fabric (Delta Lake)

Overview:
---------
This PySpark notebook ingests updated `issue` records from the Workfront API,
...
object_type = "issue"  # The Workfront object being ingested.
...
logger.info(f"Fetching issues updated since: {last_update}")
...
if not data:
    ...
    mapping_df = generate_schema_mapping_df(spark, object_type="issue", raw_to_sanitized=raw_to_sanitized)
    ...
    log_schema_to_table(spark, object_type="issue", raw_to_sanitized=raw_to_sanitized)
