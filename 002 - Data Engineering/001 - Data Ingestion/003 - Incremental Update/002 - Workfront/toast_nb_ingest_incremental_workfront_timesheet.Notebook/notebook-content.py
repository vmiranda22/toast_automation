...
"""
Workfront Timesheet Ingestion Pipeline - Microsoft Fabric (Delta Lake)

Overview:
---------
This PySpark notebook ingests updated `timesheet` records from the Workfront API,
...
- Updates the ingestion cursor based on the most recent lastUpdateDate value in the data.
...
logger.info(f"Fetching timesheets updated since: {last_update}")
...
if not data:
    ...
    mapping_df = generate_schema_mapping_df(spark, object_type="timesheet", raw_to_sanitized=raw_to_sanitized)
    ...
    log_schema_to_table(spark, object_type="timesheet", raw_to_sanitized=raw_to_sanitized)

    df_flat = df_flat.withColumn("row_hash", sha2(concat_ws("|", *df_flat.columns), 256))

    # === STEP 5: Extract and sanitize lastUpdateDate for cursor tracking ===
    ...
    ensure_raw_table_exists(spark, df_final, final_table_path, logger, label="timesheet")
    ...
    df_final_aligned.createOrReplaceTempView("staging_timesheet")
    ...
    if missing := set(df_final_aligned.columns) - set(final_schema):
        raise ValueError(f"Timesheet table schema mismatch: missing columns {missing}")
    ...
    try:
        spark.sql(f"""
            ...
            USING staging_timesheet AS source
            ON target.id = source.id
            WHEN MATCHED AND target.row_hash != source.row_hash THEN UPDATE SET *
            WHEN NOT MATCHED THEN INSERT *
        """)
    except Exception as e:
        logger.error(f"[Merge Failure] Timesheet table merge failed: {type(e).__name__} - {str(e)}")
        merge_success = False
