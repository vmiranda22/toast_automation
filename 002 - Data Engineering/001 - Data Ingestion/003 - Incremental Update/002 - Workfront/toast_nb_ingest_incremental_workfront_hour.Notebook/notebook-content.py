...
"""
Workfront Hour Ingestion Pipeline - Microsoft Fabric (Delta Lake)

Overview:
---------
This PySpark notebook ingests updated `hour` records from the Workfront API,
...
logger.info(f"Fetching hours updated since: {last_update}")
...
if not data:
    ...
    mapping_df = generate_schema_mapping_df(spark, object_type="hour", raw_to_sanitized=raw_to_sanitized)
    ...
    log_schema_to_table(spark, object_type="hour", raw_to_sanitized=raw_to_sanitized)
    ...
    ensure_raw_table_exists(spark, df_final, final_table_path, logger, label="hour")
    ...
    df_final_aligned.createOrReplaceTempView("staging_hour")
    ...
    if missing := set(df_final_aligned.columns) - set(final_schema):
        raise ValueError(f"Hour table schema mismatch: missing columns {missing}")
    ...
    try:
        spark.sql(f"""
            ...
            USING staging_hour AS source
            ON target.id = source.id
            WHEN MATCHED AND target.row_hash != source.row_hash THEN UPDATE SET *
            WHEN NOT MATCHED THEN INSERT *
        """)
    except Exception as e:
        logger.error(f"[Merge Failure] Hour table merge failed: {type(e).__name__} - {str(e)}")
        merge_success = False
